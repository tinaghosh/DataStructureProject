...
public class ConstructBinaryTreeFromPreorderAndInorderTraversal {
    ...
    Map<Integer,Integer> inOderTreeIndexMap = new HashMap<>();
    ...
    public TreeNode constructBinaryTreeFromPreorderAndInorderTraversal(int[] preOrderTree, int[] inOrderTree ){
        ...
        for(int i=0;i<inOrderTree.length;i++){
            inOderTreeIndexMap.put(inOrderTree[i],i); //preorder and inorder consist of unique values.
        }
    ...
    public TreeNode buildTree(int[] preOrderTree , int start, int end){
        ...
        int rootData =  preOrderTree[preIndex] ;
        preIndex++;
        TreeNode root = new TreeNode(rootData);
        System.out.println("rootData :"+rootData);

        int boundaryIndex = inOderTreeIndexMap.get(rootData);
        System.out.println("boundaryIndex :"+boundaryIndex);
